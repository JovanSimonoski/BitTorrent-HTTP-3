...
def get_list_of_peers(torrentfile_data):
    ...
    torrent_info_hash = hashlib.sha1(bencode_utils.bencode(torrentfile_data[b"info"])).digest()
...
def perform_handshake(torrentfile_data, peer):
    ...
    handshake_msg = length_of_protocol_string + protocol_string + reserver_bytes + torrent_info_hash + peer_id
...
def request_block(s, piece_number, block_number, data_length):
    ...
    payload = (struct.pack(">I", requested_piece_number) + struct.pack(">I", begin) + struct.pack(">I", data_length))
...
def download_piece(output_file, file_name, piece_number):
    ...
    pieces_number = len(torrent_info.piece_hashes(torrentfile_data[b"info"][b"pieces"]))
    ...
    num_blocks = full_blocks_number + 1 if final_block_length > 0 else full_blocks_number
    ...
    expected_hash = torrent_info.piece_hashes(torrentfile_data[b"info"][b"pieces"])[int(piece_number)]
...
def download(output_file, torrent_file):
    ...
    number_of_pieces = len(torrent_info.piece_hashes(torrentfile_data[b"info"][b"pieces"]))

    tmp_files = []

    for i in range(number_of_pieces):
        piece_number, output_file_tmp = download_piece("/tmp/tmp-file-piece-" + str(i), torrent_file, i)
